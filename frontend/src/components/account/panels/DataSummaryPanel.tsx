...
export const DataSummaryPanel = ({ onClose }: { onClose: () => void }) => {
  return (
    <motion.div
      variants={{
    ...
    >
      <div className="flex items-center gap-3 px-4 pb-4 border-b border-border-light dark:border-border-dark shrink-0">
        ...
        <h2 className="font-serif text-[16px] text-text-primary dark:text-text-darkPri">Data Summary</h2>
      </div>
      <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-6">
        <p className="text-sm text-text-secondary">Your data summary will appear here.</p>
      </div>
    </motion.div>
  );
};
